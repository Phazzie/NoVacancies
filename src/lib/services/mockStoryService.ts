...
class MockStoryService implements StoryService {
	...
	async getOpeningScene(_request?: OpeningSceneRequest): Promise<Scene> {
		return cloneScene(this.scenes.opening);
	}

	async getNextScene(currentSceneId: string, choiceId: string, _gameState: GameState): Promise<Scene> {
		const currentScene = this.scenes[currentSceneId];
		if (!currentScene) {
			console.error(`Scene not found: ${currentSceneId}`);
			return cloneScene(this.scenes.ending_loop);
		}

		const choice = currentScene.choices.find((candidate) => candidate.id === choiceId);
		if (!choice) {
			console.error(`Choice not found: ${choiceId} in scene ${currentSceneId}`);
			return cloneScene(this.scenes.ending_loop);
		}

		const nextScene = choice.nextSceneId ? this.scenes[choice.nextSceneId] : undefined;
		if (!nextScene) {
	...
	async getRecoveryScene(): Promise<Scene> {
		return cloneScene(this.scenes.sit_reflect);
	}

	getSceneById(sceneId: string): Scene | undefined {
		return this.scenes[sceneId] ? cloneScene(this.scenes[sceneId]) : undefined;
	}
