...
- **Builder auth smoke parity:** Treated non-HTML `/builder` requests (including Node/test `fetch` with `Accept: */*`) as JSON clients so anonymous smoke checks receive the intended 401 instead of following the browser-login redirect to a 200 page, and enabled demo auth inside the story-runtime smoke environment so login assertions exercise the intended route behavior, and the readiness config-failure payload now retains the image pipeline check for e2e contract stability.

- **Review-comment follow-up — prompt budget + context cap:** Condensed the No Vacancies system prompt by making lesson catalog references title-only, pruning repeated Oswaldo example lists, and adding prompt-budget regression coverage. Hardened `buildNarrativeContext()` so oversized derived lesson/boundary/thread state lines are trimmed after older summaries/recent prose, making the context budget a real cap under large boundary histories. Also repaired the lessons catalog syntax break between lessons 17 and 18 so unit imports compile.
...
- **Reliability & Correctness Batch (6 items):**
  ...
  - **Transport unit coverage:** Added `tests/unit/grok/transport.spec.ts` covering 7 scenarios: 200 success, 401/403 auth errors, 429 rate limit, 503 server error, `AbortError` timeout, and generic network error.
  - **Validate `storyThreadUpdates` before applying to game state:** Added `sanitizeStoryThreadUpdates()` in `sceneNormalizer.ts`. It validates each `StoryThreads` field individually (numeric, boolean, `string[]`) and silently drops unknown keys or type-mismatched values instead of forwarding a raw AI-supplied cast to the runtime. (`src/lib/server/ai/providers/grok/sceneNormalizer.ts`, `tests/unit/grok/sceneNormalizer.spec.ts`)
  - **`retryBackoffMs` env-var configurability:** Added parsing for `AI_RETRY_BACKOFF_MS` (comma-separated ms, max 4 values, each clamped 0–10 000). Falls back to `[400, 1200]` on any invalid entry. (`src/lib/server/ai/config.ts`, `tests/unit/config.spec.ts`)
  - **Removed dead `aiAuthBypass` field from `AiConfig`:** The field was always `false` (setting `AI_AUTH_BYPASS=true` throws). Removed it from the interface and the return object. Updated `demo/readiness` `security_mode` check to a static pass (guaranteed by the guard). (`src/lib/server/ai/config.ts`, `src/routes/api/demo/readiness/+server.ts`, `tests/unit/config.spec.ts`)
...
- **Readiness Telemetry Surfacing:** Readiness payloads now expose active cartridge metadata, image generation diagnostics, and image-pipeline status; home and debug surfaces render the readiness meta-grid with human-readable telemetry for operator checks.
- **Image Pipeline Instrumentation:** Image pipeline tracks request history and emits success/failure diagnostics, supports creator actions in API/e2e coverage, and settings surfaces pipeline config errors to keep creator tooling visible even when Grok images are disabled.
...
- **Story Definition Runtime Seam:** Extracted No Vacancies into a typed `StoryDefinition` / cartridge structure, added a second starter-kit story, and routed prompt generation, image paths, readiness metadata, and runtime defaults through the active story registry (`src/lib/stories/**`, `src/lib/game/imagePaths.ts`, `src/routes/api/demo/readiness/+server.ts`).
- **Narrative Context Contract Upgrade:** Replaced `recentBeats` with first-sentence `recentOpenings`, removed `arcPosition`, and upgraded transition bridges to structured before/after moments so continuity can be rendered from story-authored prose instead of canned lookup strings (`src/lib/contracts/game.ts`, `src/lib/game/narrativeContext.ts`, `src/lib/stories/no-vacancies/context.ts`).
