...
- **Builder Auth Gate + Audit Trails:** Added server-side session/role enforcement for `/builder` and `/api/builder/*`, structured 401/403 error payloads for graceful UI handling, pre-route auth checks in `hooks.server`, and builder audit metadata logging (`userId`, `draftId`, `action`) without secret leakage (`src/hooks.server.ts`, `src/lib/server/auth.ts`, `src/routes/api/builder/generate-draft/+server.ts`, `src/routes/api/builder/evaluate-prose/+server.ts`, `tests/storyEngineRuntimeSelection.js`, `README.md`).
- **Builder Auth Hardening Follow-up:** Replaced unsigned builder-session parsing with HMAC-signed `nv_session` verification (`AUTH_SESSION_SECRET`), tightened `/builder` route boundary matching, emitted explicit denied-access audit telemetry, and expanded smoke coverage to include `/api/builder/evaluate-prose`, authorized `/builder` page load, and `/builderish` non-match regression checks (`src/lib/server/auth.ts`, `src/hooks.server.ts`, `tests/storyEngineRuntimeSelection.js`, `.env.example`, `README.md`).
