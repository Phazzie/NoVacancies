...
- **Scene Contract Single-Source Validation:** Added a strict shared scene schema parser (`parseScene` / `isScene`) with typed contract errors, then wired API story-service parsing and Grok normalization through it so scene validation behavior is consistent across client/server boundaries (`src/lib/contracts/schemas/scene.ts`, `src/lib/contracts/game.ts`, `src/lib/services/storyService.ts`, `src/lib/server/ai/providers/grok.ts`, `tests/unit/contracts/sceneSchema.spec.ts`).
