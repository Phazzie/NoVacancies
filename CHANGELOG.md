...
- **Provider Capability Contract Narrowing:** Split provider contracts into explicit text-scene, image, and probe interfaces; kept a temporary backward-compatible `AiProvider` alias; narrowed provider selection helpers and route helper usage to capability-specific contracts; and added unit coverage for selection gating behavior (`src/lib/server/ai/provider.interface.ts`, `src/lib/server/ai/providers/index.ts`, `src/lib/server/ai/routeHelpers.ts`, `src/routes/api/ai/probe/+server.ts`, `tests/unit/providers/selectionContracts.spec.ts`).
