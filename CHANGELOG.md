...
- **AI Telemetry Sink Abstraction:** Added a `TelemetrySink` interface plus default `ConsoleTelemetrySink`, refactored AI telemetry emission to route through an injectable sink while preserving payload redaction behavior, and added focused unit coverage for redaction + sink swapping (`src/lib/server/ai/telemetry.ts`, `src/lib/server/ai/telemetrySink.ts`, `tests/unit/telemetry/redaction.spec.ts`, `tests/unit/telemetry/sink.spec.ts`, `src/lib/server/ai/index.ts`).
