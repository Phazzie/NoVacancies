...
- **Grok Provider Modularization:** Split Grok provider internals into dedicated transport, retry policy, scene parser, and scene normalizer modules; `grok.ts` now orchestrates these components while preserving bounded two-attempt parse recovery behavior. Added focused unit coverage for parser extraction/recovery, retry backoff/decisions, and scene normalization behavior (`src/lib/server/ai/providers/grok.ts`, `src/lib/server/ai/providers/grok/transport.ts`, `src/lib/server/ai/providers/grok/retryPolicy.ts`, `src/lib/server/ai/providers/grok/sceneParser.ts`, `src/lib/server/ai/providers/grok/sceneNormalizer.ts`, `tests/unit/grok/*.spec.ts`).
