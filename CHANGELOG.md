...
- **Builder Full-Draft QA Workflow:** Added full-draft evaluation in the builder service (AI-first with deterministic fallback rubric), shipped `POST /api/builder/evaluate-draft`, exposed publishability thresholds/readiness state, and added a “Run Draft QA” UI with grouped blocker/warning/info findings plus jump links to affected draft fields (`src/lib/server/ai/builder.ts`, `src/lib/stories/types.ts`, `src/routes/api/builder/evaluate-draft/+server.ts`, `src/routes/builder/+page.svelte`, `src/hooks.server.ts`, `src/app.css`, `tests/unit/builderHelpers.spec.ts`, `tests/narrative/narrativeQuality.test.js`).
