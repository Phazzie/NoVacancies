...
- **Grok Provider Modularization:** Split Grok provider internals into dedicated transport, retry-policy, scene-parser, and scene-normalizer modules, then refactored `grok.ts` into orchestration-only flow while preserving bounded two-level parse recovery semantics and existing runtime behavior (`src/lib/server/ai/providers/grok.ts`, `src/lib/server/ai/providers/grok/transport.ts`, `src/lib/server/ai/providers/grok/retryPolicy.ts`, `src/lib/server/ai/providers/grok/sceneParser.ts`, `src/lib/server/ai/providers/grok/sceneNormalizer.ts`, `tests/unit/grok/*.spec.ts`).
